...
def load_data(directory):
    ...
    """
    # Load people
    with open("E:\degrees\large\people.csv", encoding="utf-8") as f:
    ...
    # Load movies
    with open("E:\degrees\large\movies.csv", encoding="utf-8") as f:
        reader = csv.DictReader(f)
        for row in reader:
            movies[row["id"]] = {
                "title": row["title"],
    ...
    # Load stars
    with open("E:\degrees\large\stars.csv", encoding="utf-8") as f:
        reader = csv.DictReader(f)
        for row in reader:
            try:
...
def main():
    ...
    directory = sys.argv[1] if len(sys.argv) == 2 else "large"#这一段if是啥意思啊
